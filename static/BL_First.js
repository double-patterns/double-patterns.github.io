...
function entry(pat){
	...
	blist.forEach(function(element){
	    if(element == cur_pattern){
		timer("BLF",Array.from(lock1.getPattern()).map(x=>x-1).join('') + " " + Array.from(lock2.getPattern()).map(x=>x-1).join(''))
		lock_first();
		bl_attempts.push(cur_pattern);
		$('#blacklistModal').modal({ keyboard: false, backdrop: 'static' });
		document.getElementById("exampleModalLabel").innerHTML="The first pattern of your Double Pattern can be easily guessed."
		bl = true
	    }
	});
	if(!bl){
