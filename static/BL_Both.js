...
var MSG_BLACK = "Your chosen pattern is a commonly utilized sequence, please choose another.";
...
var RES_HTML = "<button type='button' onclick='user_reset()' class='btn inter'>Reset</button>";
var NORES_HTML = "<button type='button' class='btn inter nobut'>Reset</button>";
var PRACT_HTML = "<button type='button' onclick='pract_res()' class='btn inter'>More Practice</button>";
...
function entry(pat){
	...
	timer("ERL",Array.from(lock1.getPattern()).map(x=>x-1).join('') + " " + Array.from(lock2.getPattern()).map(x=>x-1).join(''))
	...
	timer("ERR",Array.from(lock1.getPattern()).map(x=>x-1).join('') + " " + Array.from(lock2.getPattern()).map(x=>x-1).join(''))
	lock_second();
	...
	bl = false
	blist.forEach(function(element){
	    if(element[0] == pattern1 && element[1] == cur_pattern){
		timer("BLB",Array.from(lock1.getPattern()).map(x=>x-1).join('') + " " + Array.from(lock2.getPattern()).map(x=>x-1).join(''))
		lock_second();
		bl_attempts.push(pattern1 + " " + cur_pattern);
		$('#blacklistModal').modal({ keyboard: false, backdrop: 'static' });
		document.getElementById("exampleModalLabel").innerHTML="The Double Pattern you created can be easily guessed."
		
		//feedback(MSG_BLACK,NO_BUT);
		//setTimeout(function() {pat_reset();}, 2000);
		bl = true
	    }
	});
	if(!bl){
...
function entry2(pat){
	...
	//Either pattern incorrect
	    ...
	    timer("ERC",Array.from(lock1.getPattern()).map(x=>x-1).join('') + " " + Array.from(lock2.getPattern()).map(x=>x-1).join(''))
	    both_wrong();
...
function entry_continue(){
    timer("CON",Array.from(lock1.getPattern()).map(x=>x-1).join('') + " " + Array.from(lock2.getPattern()).map(x=>x-1).join(''))
    ...
    document.getElementById("bl_attempts").value = JSON.stringify(bl_attempts); //keep it as a list
...
function user_reset(){
    timer("USR",Array.from(lock1.getPattern()).map(x=>x-1).join('') + " " + Array.from(lock2.getPattern()).map(x=>x-1).join(''))
    pat_reset();
}

function pract_res(){
    timer("MOR",Array.from(lock1.getPattern()).map(x=>x-1).join('') + " " + Array.from(lock2.getPattern()).map(x=>x-1).join(''))
    pat_reset();
}
...
function timer(event,data){
    times.push([event,(performance.now()/1000).toFixed(2),data]);
}
