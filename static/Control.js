...
function recall(){
	...
	//Both patterns are correct
        if(firstPatt == 1 && cmp_pat(cur_pattern,pattern2)){
	    timer("COR",pattern1+" "+cur_pattern)
	...
	//Second pattern is incorrect
	    ...
	    timer("REC",pattern1+" "+cur_pattern)
	    lock_second();
	    both_wrong();
	    recall_check();
	    feedback(MSG_BAD_RECALL, SAME_BUT);
...
function recall_check(){
    if(document.getElementById("function").value != "recall")
	setTimeout(function() {feedback(MSG_ENTRY,NO_BUT);}, 1500);
    //Out of attempts
    else if(attempts0.length >= 5){
